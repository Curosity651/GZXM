...
export function RequirementChecklist({ requirements, ownerType, ownerId, topicId, editable }: {
  ...
  const save = (requirement: ArchiveRequirement, patch: Partial<ArchiveSubmission>) => {
    ...
    state.saveArchiveSubmission({
      id: current?.id ?? `archive-${Date.now()}-${requirement.id}`, requirementId: requirement.id, ownerType, ownerId, topicId,
  ...
  const submit = (requirement: ArchiveRequirement) => {
    ...
    state.submitArchive(current.id, state.currentUser!.id); message.success(ownerType === 'PROJECT_PUBLIC' ? '已提交项目技术负责人终审' : '已提交科研助理初审');
  };
  return <>
    <Space style={{ marginBottom: 16 }}><b>归档进度</b><Progress percent={completion.rate} style={{ width: 240 }} /><span>{completion.completed}/{completion.required} 项终审通过</span></Space>
    <Table rowKey="id" pagination={false} dataSource={requirements} columns={[
      ...
      { title: '适用性', width: 230, render: (_, row) => {
        ...
        return <Space direction="vertical" size={4}><Select disabled={!editable || row.requirementKind === 'REQUIRED' || ['初审中', '终审中', '已通过'].includes(current?.status ?? '')} value={applicability} style={{ width: 130 }} onChange={(value) => save(row, { applicability: value })} options={[{ label: '待确认', value: 'PENDING' }, { label: '适用', value: 'APPLICABLE' }, { label: '不适用', value: 'NOT_APPLICABLE' }]} />{applicability === 'NOT_APPLICABLE' && <Input disabled={!editable} value={current?.nonApplicableReason} placeholder="不适用理由" onChange={(event) => save(row, { applicability, nonApplicableReason: event.target.value })} />}</Space>;
      } },
      { title: '材料', width: 180, render: (_, row) => { const current = getSubmission(row.id); return <Space direction="vertical"><span>{current?.fileIds.length ? `${current.fileIds.length} 个 Mock 文件` : '尚未上传'}</span>{editable && !['初审中', '终审中', '已通过'].includes(current?.status ?? '') && <Upload showUploadList={false} beforeUpload={(file) => { save(row, { fileIds: [...(current?.fileIds ?? []), `mock-${file.uid}`], applicability: current?.applicability === 'NOT_APPLICABLE' ? 'NOT_APPLICABLE' : 'APPLICABLE' }); message.success(`已记录文件：${file.name}`); return false; }}><Button size="small" icon={<UploadOutlined />}>选择文件</Button></Upload>}</Space>; } },
      { title: '状态', width: 110, render: (_, row) => <StatusTag status={getSubmission(row.id)?.status ?? '未提交'} /> },
      { title: '操作', width: 100, render: (_, row) => { const current = getSubmission(row.id); return editable && !['初审中', '终审中', '已通过'].includes(current?.status ?? '') ? <Button type="link" icon={<SendOutlined />} onClick={() => submit(row)}>提交</Button> : '—'; } },
    ]} />
  </>;
}
