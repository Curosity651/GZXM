...
export function ReportManagementPage() {
  ...
  const tasks = useMemo(() => state.reportTasks.filter((item) => item.topicId === user.topicId), [state.reportTasks, user.topicId]);
  ...
  const columns = [
    ...
    { title: '操作', width: 120, render: (_: unknown, row: ReportTask & { report?: ProgressReport }) => <Button type="link" icon={<EditOutlined />} disabled={row.report ? !isReportEditable(row.report.status) : false} onClick={() => openEdit(row)}>填报/查看</Button> },
  ];
  return <>
    <PageHeader title="月报与季报" description={`${topic?.name ?? '本课题'} · 月报每月 30 日前，季报于 3/10、6/10、9/10、12/10 前提交。`} />
    ...
    <Drawer width={760} title={editingTask?.reportType === 'MONTHLY' ? '课题月报填报' : '课题季报填报'} open={drawer} onClose={() => setDrawer(false)} extra={<Space><Button onClick={save}>保存草稿</Button><Button type="primary" icon={<SendOutlined />} onClick={submit}>提交初审</Button></Space>}>
      {editingTask && <Space style={{ marginBottom: 16 }}><Text strong>{topic?.name}</Text><Tag>{editingTask.deadline} 截止</Tag>{editingReport && <StatusTag status={editingReport.status} />}</Space>}
