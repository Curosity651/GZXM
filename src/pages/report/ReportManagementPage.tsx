...
import { isReportOpen, isReportOverdue } from '../../domain/reporting';
...
export function ReportManagementPage() {
  ...
  const leadTopicIds = state.topics.filter((item) => isTopicLead(user, item.id, state.topicMemberships)).map((item) => item.id);
  const primaryTopicId = leadTopicIds[0];
  ...
  const tasks = useMemo(() => state.reportTasks.filter((task) => leadTopicIds.includes(task.topicId)), [leadTopicIds, state.reportTasks]);
  ...
  const columns = [
    ...
    { title: '开放日期', dataIndex: 'openDate', width: 120 }, { title: '截止日期', dataIndex: 'deadline', width: 120 },
    ...
    { title: '操作', width: 120, render: (_: unknown, row: ReportTask & { report?: ProgressReport }) => <Button type="link" icon={<EditOutlined />} disabled={!isReportOpen(row) || (row.report ? !isReportEditable(row.report.status) : false)} onClick={() => openEdit(row)}>{isReportOpen(row) ? '填报/查看' : '尚未开放'}</Button> },
  ];
  return <>
    <PageHeader title="月季报提交" description={canSubmit ? `${topic?.name ?? '牵头课题'} · 每个课题每期只由牵头单位提交一份报告。` : '当前账号没有月季报提交权限。'} />
