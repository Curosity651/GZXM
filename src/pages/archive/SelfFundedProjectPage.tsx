...
export function SelfFundedProjectPage() {
  ...
  return <>
    ...
    <Row gutter={[16, 16]}>{projects.map((project) => { const completion = archiveCompletion(requirementsFor(project), state.archiveSubmissions.filter((item) => item.ownerType === 'SELF_FUNDED' && item.ownerId === project.id)); return <Col xs={24} xl={12} key={project.id}><Card hoverable title={<Space><FolderOpenOutlined /><span>{project.name}</span></Space>} extra={<StatusTag status={project.status} />} actions={[<Button type="link" onClick={() => setSelected(project)}>进入归档</Button>]}><Space direction="vertical" style={{ width: '100%' }}><Space><Tag color="blue">{project.projectType}</Tag><Tag>{project.code}</Tag></Space><div>项目负责人：{project.principalName}　实施单位：{project.implementingUnit}</div><Progress percent={completion.rate} status={completion.rate < 50 ? 'exception' : 'active'} /><div>{completion.completed}/{completion.required} 项材料终审通过</div></Space></Card></Col>; })}</Row>
