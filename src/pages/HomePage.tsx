...
export function HomePage() {
  ...
  const archiveDone = visibleArchiveSubmissions.filter((item) => ['已通过', '已归档'].includes(item.status)).length;
  ...
  return <>
    ...
    <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
      <Col xs={24} xl={16} style={{ display: 'flex' }}><Card title="课题执行概览" extra={<Tag color="blue">已归档项计入</Tag>} style={{ width: '100%', height: '100%' }}>
