...
import { accessibleTopics } from '../../domain/topic-access';

export function IndicatorConfigPage() {
  const state = useAppStore(); const user = state.currentUser!; const navigate = useNavigate();
  const visibleTopics = accessibleTopics(user, state.topics, state.topicMemberships); const canManageTopics = canPerform(user, state.roles, 'topic.manage');
