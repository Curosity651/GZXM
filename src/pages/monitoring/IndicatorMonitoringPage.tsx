...
export function IndicatorMonitoringPage() {
  ...
  const effective = state.achievements.filter((item) => visibleTopicIds.has(item.topicId) && (item.status === '已生效' || item.status === '审批通过'));
  ...
  return <>
    ...
    <Card title="课题指标完成情况"><Table rowKey="topicId" dataSource={summaries} pagination={false} columns={[
