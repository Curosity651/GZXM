...
describe('归档 Store 流程', () => {
  it('课题账号可以在本课题下新建配套自筹项目', () => {
    ...
    store.getState().addSelfFundedProject({ id: 'sf-new', topicId: 't1', code: 'ZC-KJ-009', name: '配套验证项目', projectType: '科技项目', principalName: '李工', implementingUnit: '广西电网', status: '筹备中', templateSnapshotId: 'tpl-tech-v1' }, 'user-topic-1');
    expect(store.getState().selfFundedProjects.some((item) => item.id === 'sf-new')).toBe(true);
  });

  it('课题材料完成两级审批后变为已通过', () => {
    const store = createAppStore();
    store.getState().saveArchiveSubmission({ id: 'as-new', requirementId: 'ar-4', ownerType: 'TOPIC_NATIONAL', ownerId: 't1', topicId: 't1', applicability: 'APPLICABLE', status: '草稿', fileIds: ['file-1'], version: 1, updatedAt: '2026-09-09' });
    store.getState().submitArchive('as-new', 'user-topic-1');
